...
export default async function formatVideo(
  video: any,
  speedDate: boolean = false,
) {
  try {
    ...
    if (
      ...
      // Duration formating
      if (video.lengthText) {
        if (video.lengthText.hasOwnProperty('simpleText')) {
          //For more videos
      ...
      if (speedDate && video.publishedTimeText) {
        ...
        publishedAt = (await getVideoDate(id)) || new Date();
      }
