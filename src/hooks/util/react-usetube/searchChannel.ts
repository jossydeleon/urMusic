...
export default async function searchChannel(
  terms: string,
  token?: string,
  apikey?: string,
) {
  try {
    ...
    if (!token) {
      const data = await getData(
        'https://m.youtube.com/results?sp=EgIQAg%253D%253D&search_query=' +
          encodeURI(terms),
      );
      apikey = data.apikey;
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i].compactChannelRenderer || items[i].channelRenderer) {
        const item = items[i].compactChannelRenderer
          ? items[i].compactChannelRenderer
          : items[i].channelRenderer;
        item.name = items[i].compactChannelRenderer
          ? item.title.runs[0].text
          : item.title.simpleText;
        let avatar = item.thumbnail?.thumbnails[0].url || '';
        let avatarId = avatar.substring(
          avatar.lastIndexOf('ytc/') + 4,
          avatar.lastIndexOf('=s'),
        );
        const nbSubscriber: number = formatYoutubeCount(
          item.subscriberCountText?.accessibility.accessibilityData.label ||
            '0',
        );
        const nbVideo: number = formatYoutubeCount(
          item.videoCountText?.runs[0]?.text || '0',
        );
        channels.push({
          name: item.name,
          ...
          channel_avatar_small:
            'https://yt3.ggpht.com/ytc/' + avatarId + '=s80',
          channel_avatar_medium:
            'https://yt3.ggpht.com/ytc/' + avatarId + '=s200',
          channel_avatar_large:
            'https://yt3.ggpht.com/ytc/' + avatarId + '=s800',
        });
      } else if (
        items[i].didYouMeanRenderer ||
        items[i].showingResultsForRenderer
      ) {
        let item: any;
