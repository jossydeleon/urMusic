...
const useHelpers = () => {
  ...
  const capitalize = (title: string): string => {
    if(!title) return ''
    const [firstLetter, ...restOfWord] = title.toLowerCase().split('"').join('');
  ...
  const secondsToTimerFormat = (seconds: string) => {
    ...
    return new Date(parse * 1000).toISOString().substr(11, 8).replace(/^0(?:0:0?)?/, '')
  }
  ...
  const formatDateToTimeAgo = (date: Date) => {
    if(!date) return moment(new Date()).fromNow();
